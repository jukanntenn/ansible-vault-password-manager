...
passphrase correctly and subsequent non-interactive calls (`avpm --vault-id dev`
from Ansible) will work without prompting.
...
merely annoying; for Ansible's non-interactive `avpm --vault-id <id>` calls it
is fatal (exit code 5, `Locked`).
